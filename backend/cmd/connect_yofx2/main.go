...
func main() {
	log.Println("╔═══════════════════════════════════════════════════════════╗")
	log.Println("║      XAUUSD Subscription via FIX 4.4 YoFX2 Session       ║")
	log.Println("╚═══════════════════════════════════════════════════════════╝")
	...
	// Step 4: Subscribe to XAUUSD
	log.Println("\n[4/5] Subscribing to XAUUSD market data...")
	mdReqID, err := gateway.SubscribeMarketData("YOFX2", "XAUUSD")
	if err != nil {
		log.Fatalf("❌ Failed to subscribe XAUUSD: %v", err)
	}
	log.Printf("✅ Subscription request sent! MDReqID: %s", mdReqID)

	// Step 5: Monitor market data feed
	log.Println("\n[5/5] Monitoring XAUUSD market data (60 seconds)...")
	...
	go func() {
		for {
			...
			case md := <-gateway.GetMarketData():
				if md.Symbol == "XAUUSD" {
					tickCount++
					log.Printf("✅ [%3d] XAUUSD Market Data:", tickCount)
			...
			case <-ticker.C:
				...
				if tickCount == 0 {
					log.Printf("⏳ Waiting for XAUUSD data... (Ticks: %d, Rejects: %d)", tickCount, rejectCount)
				}
			}
		}
	}()
	...
	if tickCount > 0 {
		log.Printf("✅ SUCCESS: Received %d XAUUSD ticks from FIX 4.4 YoFX2", tickCount)
		log.Println()
		log.Println("📊 Next Steps:")
		log.Println("   • XAUUSD data is now flowing through the backend")
		log.Println("   • Check backend/data/ticks/XAUUSD/ for persisted ticks")
		log.Println("   • Connect to ws://localhost:7999/ws to stream live quotes")
	} else {
		log.Printf("⚠️  No XAUUSD ticks received (Rejects: %d)", rejectCount)
		log.Println()
...
func checkTickFiles() {
	...
	resp, err := client.Get("http://localhost:7999/api/v1/ticks/XAUUSD?limit=5")
	if err != nil {
	...
	if len(ticks) > 0 {
		log.Printf("   📁 Found %d recent XAUUSD ticks in backend storage", len(ticks))
		lastTick := ticks[len(ticks)-1]
		log.Printf("       Latest: Bid=%.2f Ask=%.2f LP=%s",
			lastTick["bid"], lastTick["ask"], lastTick["lp"])
	} else {
		log.Println("   📁 No XAUUSD ticks found in backend storage")
	}
}
